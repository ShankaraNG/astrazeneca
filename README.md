...
- The raw DepMap / HPA / CCLE data files placed under `CelllineSelector/data/raw/` as described
  in the **Data setup** section below. Only the raw data is needed by hand the pipeline generates everything else.

## Data setup

Only the **raw data** needs to be supplied by hand. Everything else in the
`data/` directory — `cleaned_data/`, `cosine_matrix/`, `kmeans_data/`,
`knn_data/`, `lookup/`, `mofa_data/`, `scoring/` — is **generated by
the `ml_build` pipeline** and should not be created manually.

Place the raw files in a `data/raw/` directory at the project root, arranged
into the five category subfolders below. The pipeline reads from these paths,
so the folder names and file numbering must match exactly.
...
└── data/
    └── raw/
        ...
        │   ├── 2_DepMap_OmicsExpressionAllGenesTPMLogp1Profile
        │   ├── 3_GEOexpression.txt
        │   └── 4_Harmonized_MS_CCLE_Gygi_subsetted
        ├── gene properties/
...
> **Only `CelllineSelector/data/raw/` is required to start.** Running the offline pipeline
> (`python -m ml_build.main`) reads these files and writes all the other
> `data/` subdirectories and the trained models in `models/`. Do not create
> or edit the generated folders by hand.

The 14 numbered files span the four omics layers plus the nomenclature and
BioMart references used for identifier harmonisation:

| Category | Files | Role |
|---|---|---|
| Gene expression | 1–4 | HPA RNA, DepMap RNA, GEO, MS proteomics |
| Gene properties | 5–6 | Gene fusions, somatic mutations |
| Nomenclature | 7–11 | Cellosaurus, DepMap profiles/sample info, GEO info, HPA name map |
| Non-gene expression | 12–14 | CCLE metabolomics, CCLE miRNA, global genomic signatures |
| BioMart reference | `mart_export.txt` |
