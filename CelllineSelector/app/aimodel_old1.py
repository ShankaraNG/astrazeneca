...
def _build_messages(final_top10_df, exclusionapplied, diseasename,
    ...
    try:
        ...
        for rank, (_, row) in enumerate(df.iterrows(), start=1):
            name = str(row.get('cell_line_name', 'Unknown'))
            disease = str(row.get('primary_disease', 'Unknown'))
            subgroup = str(row.get('biological_sub_group', 'Not specified'))
            
            outline_slots.append(
                f"Rank {rank} - {name} ({disease})\n"
                f"[Write 1 narrative paragraph evaluating metrics for Rank {rank}, and conclude the paragraph by detailing the biological context of {name} (e.g., primary disease origin: {disease}, biological subgroup: {subgroup}, known genomic/cancer characteristics, or disease model relevance).]"
            )
        ...
        system_prompt = (
            ...
            "4. HUMAN NARRATIVE COMPARISON: Explain metric relationships in natural language.\n"
            "5. BIOLOGICAL CONTEXT AT PARAGRAPH END: Conclude EVERY rank paragraph with 2-3 sentences explaining what the cell line is biologically (tissue/disease origin, subtype, or notable oncogenic characteristics).\n"
            "6. NO MARKDOWN HEADERS: Do not use '#' or '##'. Use simple line headers."
        )
        ...
        user_prompt = (
            ...
            f"For each cell line, discuss its scoring metrics and conclude the paragraph with its biological context and cancer relevance. "
...
def ollama_chat(system_prompt, user_prompt, model='llama3.2:3b',
    ...
    """Local Ollama chat call configured for strict structural compliance."""
    try:
        import requests
