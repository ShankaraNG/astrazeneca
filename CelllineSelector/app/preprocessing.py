...
def extract_with_fallback(data_df, id_list, id_kind, prefix, filename):
    ...
    before = len(result)
    result = result.dropna(subset=['ModelID'])
    result = result.drop_duplicates(subset='ModelID', keep='first')
    removed = before - len(result)
    if removed:
        log.warning(f"{filename}: removed {removed} row(s) with null/duplicate ModelID "
                    f"(kept {len(result)} unique cell line(s))")
    for i in missing:
        result[f"{prefix}{i}"] = np.nan
...
def datacombiner(data_df):
    try:
        ...
        masterlookup_df = masterlookup_df.dropna(subset=['ModelID']).drop_duplicates(subset='ModelID', keep='first')
...
def alternativedatacombiner(data_df):
    try:
        ...
        masterlookup_df = masterlookup_df.dropna(subset=['AlternativeModelID']).drop_duplicates(subset='AlternativeModelID', keep='first')
...
def filtercelllineforcelllinename(data_df):
    try:
        log.info(f"Getting the master look up table")
        masterlookup_df = ut.cleaned_data_reader('lookup', 'master', 'master_lookup.csv')
        if masterlookup_df is None or masterlookup_df.empty:
            raise ValueError("Mapping file 'master_lookup.csv' is missing or empty.")
        log.info(f"Filtering the required columns")
        required_columns = ['ModelID', 'CCLE_Name', 'cell_line_name', 'stripped_cell_line_name']
        keep = [c for c in required_columns if c in masterlookup_df.columns]
        masterlookup_df = masterlookup_df[keep]
        log.info(f"Mergining the data with the master table")
        data_df = data_df.merge(masterlookup_df, on='ModelID', how='left')
        log.info(f"Droping the rows which have null value")
        data_df = data_df.dropna(subset=['CCLE_Name', 'cell_line_name', 'stripped_cell_line_name'], how='any')
        log.info(f"Removing the columns which are not needed")
        data_df = data_df.drop(columns=['CCLE_Name', 'cell_line_name', 'stripped_cell_line_name'])
        return data_df
    except Exception as e:
        log.error(f"Error combining master data into alternatives: {e}")
        raise

def getmetabolomnicdata(data_df):
    try:
        log.info(f"Getting the metabolomnic data")
        metabolomics_df = ut.cleaned_data_reader('cleaned_data', 'non gene expression', '12_CCLE_metabolomics_20190502.csv')
        if metabolomics_df is None or metabolomics_df.empty:
            raise ValueError("file '12_CCLE_metabolomics_20190502.csv' is missing or empty.")
        genelist = set(data_df['ModelID'].dropna())
        matched_data_df = metabolomics_df[metabolomics_df['ModelID'].isin(genelist)]
